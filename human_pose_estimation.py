...
def pick_side(points, conf, left_idxs, right_idxs, thr=0.3):
    li = [COCO[i] for i in left_idxs]
    ri = [COCO[i] for i in right_idxs]
    lscore = np.mean([conf[i] for i in li if conf[i] is not None])
    rscore = np.mean([conf[i] for i in ri if conf[i] is not None])
    side = "L" if (lscore >= rscore) else "R"

    if max(lscore, rscore) < thr:
        side = None
    return side
...
def draw_poses(img, poses, point_score_threshold, output_transform, skeleton=default_skeleton, draw_ellipses=False):
    ...
    if poses.size != 0:
        ...
        pts = output_transform.scale(pts).astype(int)

        def P(name): return tuple(pts[COCO[name]])
        def C(name): return float(conf[COCO[name]])

        need = ["L_HIP","R_HIP","L_KNEE","R_KNEE","L_ANK","R_ANK","L_SHO","R_SHO"]
        if all(C(n) >= 0.3 for n in need):
            side = pick_side(pts, conf,
                             left_idxs=["L_HIP","L_KNEE","L_ANK"],
                             right_idxs=["R_HIP","R_KNEE","R_ANK"],
                             thr=0.3)
            if side is not None:
                side_text = side
                HIP  = P(f"{side}_HIP")
                KNEE = P(f"{side}_KNEE")
                ANK  = P(f"{side}_ANK")
                SHO_L, SHO_R = P("L_SHO"), P("R_SHO")
                ANK_L, ANK_R = P("L_ANK"), P("R_ANK")

                knee_angle = angle_deg(HIP, KNEE, ANK)

                shoulder_mid = ((SHO_L[0]+SHO_R[0])//2, (SHO_L[1]+SHO_R[1])//2)
                ankle_mid    = ((ANK_L[0]+ANK_R[0])//2, (ANK_L[1]+ANK_R[1])//2)
                denom = max(1, ankle_mid[1] - shoulder_mid[1])
                hip_height_norm = (HIP[1] - shoulder_mid[1]) / denom

                txt_angle = f"{knee_angle:.0f}" if knee_angle is not None else "NA"
                txt_hiph  = f"{hip_height_norm:.2f}"

                if knee_angle is not None:
                    DOWN_ANGLE, UP_ANGLE = 120.0, 160.0
                    DOWN_H,     UP_H     = 0.60,  0.45
                    down_cond = (knee_angle <= DOWN_ANGLE) or (hip_height_norm >= DOWN_H)
                    up_cond   = (knee_angle >= UP_ANGLE)   and (hip_height_norm <= UP_H)
                    if state == "UP" and down_cond:
                        state = "DOWN"
                    elif state == "DOWN" and up_cond:
                        state = "UP"
                        squat_count += 1

    cv2.putText(img, f"Angle:{txt_angle}  HipH:{txt_hiph}  Side:{side_text}",
                (10, 70),  cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,255), 2)
    cv2.putText(img, f"Squats: {squat_count}  State:{state}",
        ...
        for i, j in skeleton:
            if points_scores[i] > point_score_threshold and points_scores[j] > point_score_threshold:
                if draw_ellipses:
                    ...
                    polygon = cv2.ellipse2Poly(tuple(middle),
                    ...
                    cv2.line(img_limbs, tuple(points[i]), tuple(points[j]),
                             color=colors[j], thickness=stick_width)
